...
bool DrawNode::init()
{
	...
	setShaderProgram(ShaderCache::sharedShaderCache()->programForKey(kCCShader_PositionLengthTexureColor));

	ensureCapacity(512);

#if CC_TEXTURE_ATLAS_USE_VAO    
	glGenVertexArrays(1, &_uVao);
	ccGLBindVAO(_uVao);
#endif
...
void DrawNode::ensureCapacity(unsigned int count)
{
	if (_nBufferCount + count > _uBufferCapacity)
	{
		...
		_pBuffer = (ccV2F_C4B_T2F*)realloc(_pBuffer, _uBufferCapacity * sizeof(ccV2F_C4B_T2F));
	}
}
...
void DrawNode::drawDot(const Point& pos, float radius, const ccColor4F& color)
{
	...
	ccV2F_C4B_T2F_Triangle* triangles = (ccV2F_C4B_T2F_Triangle*)(_pBuffer + _nBufferCount);
