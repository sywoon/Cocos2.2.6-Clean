...
CCImage::CCImage(const CCImage& rImg)
{
	...
	m_nHeight = rImg.m_nHeight;
	m_nBitsPerComponent = rImg.m_nBitsPerComponent;
	m_bHasAlpha = rImg.m_bHasAlpha;
...
#define PNGSIGSIZE  8
	...
	do
	{
		...
		png_set_error_fn(png_ptr, &error_parameters, pngtest_error,
			pngtest_warning);
